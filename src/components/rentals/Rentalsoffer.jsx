...
const Rentalsoffer = () => {
    return (
        <div className="rental-offer-wrapper relative z-[30] pt-[35vh] tab:pt-[30vh] pb-[20vh] tab:pb-[10vh] mob:pb-[10vh] w-full flex flex-col items-center">
...
const RentalOfferLower = () => {
    ...
    return (
        <div className="offers-wrap relative pt-[7vh] tab:pt-[5vh] mob:pt-[5vh] border-t-[0.1px] bg-white border-t-[#12121280] w-full flex mob:flex-col items-start justify-between mob:gap-[10vw]">
            <div className="left w-[30%] mob:w-full">
                <p className='text-black text-[17px] tab:text-[14px]'>
                    What you can expect:
                </p>
            </div>
            <div className="right flex justify-end items-start flex-wrap gap-x-[25px] mob:gap-x-[30px] gap-y-[25px] mob:gap-y-[30px]">
                {
                    cards.map((elem, i) => {
                        return <Card value1={elem.value1} style={elem.style} value2={elem.value2} num={i+1} keyrefer={`offer_card_${i+1}`} key={i}/>
                    })
                }
            </div>
        </div>
